...
class TestCertificateCoversBothEndings:
    """The draw case was modelled but not carried through to the artefact.

    A third party re-running the UNSAT from the certificate could only check
    half the claim, and the summary fields still described the checkmate route
    ...
    def certificate(self):
        ...
        path = Path(self.CERTIFICATE) / "certificate.json"
        if not path.exists():
            pytest.skip("run scripts/certify.py")
        return json.loads(path.read_text(encoding="utf-8"))

    def test_a_model_is_exported_for_every_shape_and_ending(self):
        from pathlib import Path

        from long_chess.model import ENDINGS, all_shapes

        models = Path(self.CERTIFICATE) / "models"
        if not models.exists():
            pytest.skip("run scripts/certify.py")
        for ending in ENDINGS:
            for shape in all_shapes(4):
                name = str(shape).replace(" ", "")
                assert (models / ending / f"{name}.pb.txt").exists(), (ending, name)

    def test_no_stale_models_from_an_earlier_layout(self):
        """The flat files the first version wrote are gone, not left behind."""
        from pathlib import Path

        models = Path(self.CERTIFICATE) / "models"
        if not models.exists():
            pytest.skip("run scripts/certify.py")
        assert list(models.glob("*.pb.txt")) == []

    def test_the_claim_records_both_endings(self):
        claim = self.certificate()["claim"]
        assert set(claim["endings_covered"]) == {"checkmate", "draw"}
        assert claim["minimum_switches_by_ending"] == {"checkmate": 3, "draw": 3}

    def test_the_bound_terms_show_both_routes_reaching_118(self):
        terms = self.certificate()["bound_terms"]
        assert terms["checkmate"]["captures"] == 29
        assert terms["checkmate"]["closing_segment"] == 1
        assert terms["draw"]["captures"] == 30
        assert terms["draw"]["closing_segment"] == 0
        assert terms["checkmate"]["total"] == terms["draw"]["total"] == 118

    def test_decisions_cover_every_shape_under_every_ending(self):
        decisions = self.certificate()["decisions"]
        assert len(decisions) == 16
        assert {d["ending"] for d in decisions} == {"checkmate", "draw"}
        assert all(d["agree"] for d in decisions)

    def test_the_attained_game_points_inside_the_certificate(self):
        """It used to point at data/longest.pgn — Tom 7's reference, not the
        game this certificate actually verified."""
        assert "longest.pgn" in self.certificate()["claim"]["attained_by"]
        assert not self.certificate()["claim"]["attained_by"].startswith("data/")

    def test_the_dead_position_caveat_states_the_right_direction(self):
        """It used to say the gap 'only shortens games', which is backwards."""
        caveats = " ".join(self.certificate()["caveats"])
        assert "only shortens games" not in caveats
        assert "ends games later than FIDE" in caveats


class TestCertificateProvenance:
    """A certificate a third party cannot re-run is a claim, not evidence.

    The first version recorded a commit SHA with no check that the worktree was
    clean, no lockfile hash, no per-artefact hash, and no solver parameters —
    so a reader had no way to tell what produced the numbers or to reproduce
    them.
    """
    ...
    def load(self, name: str):
        ...
        path = Path(self.CERTIFICATE) / name
        if not path.exists():
            pytest.skip("run scripts/certify.py")
        return json.loads(path.read_text(encoding="utf-8"))

    def test_the_worktree_was_clean_when_it_was_produced(self):
        """Otherwise `source_commit` describes code that was not what ran."""
        provenance = self.load("certificate.json")["provenance"]
        assert provenance["worktree_clean"] is True
        assert provenance["worktree_dirt"] == ""

    def test_the_source_is_pinned_by_commit_and_tree(self):
        provenance = self.load("certificate.json")["provenance"]
        assert len(provenance["source_commit"]) == 40
        assert len(provenance["source_tree_hash"]) == 40

    def test_the_environment_and_inputs_are_pinned(self):
        provenance = self.load("certificate.json")["provenance"]
        assert provenance["python"].startswith("3.")
        assert provenance["ortools"]
        assert provenance["chess"]
        assert provenance["seed"] == 1
        assert provenance["solver"]["max_time_in_seconds"] > 0
        for name in ("uv.lock", "pyproject.toml", "data/skeleton.json"):
            assert len(provenance["inputs"][name]) == 64

    def test_every_artefact_is_hashed(self):
        artefacts = self.load("certificate.json")["artefacts"]
        assert len(artefacts["longest.pgn"]) == 64
        assert len(artefacts["skeleton.json"]) == 64
        assert len(artefacts["longest.trace.tsv"]) == 64
        protos = [name for name in artefacts if name.endswith(".pb.txt")]
        # Eight shapes × two endings, each with and without the home-rank
        # lemma, so the no-lemma column can be re-run too.
        assert len(protos) == 32
        assert len([name for name in protos if ".no-lemma." in name]) == 16
        assert all(len(artefacts[name]) == 64 for name in protos)

    def test_the_certificate_says_which_artefacts_are_not_committed(self):
        """A hash with no file behind it looks like a broken certificate. The
        trace is 1.5MB and gitignored; the certificate now says so rather than
        leaving a fresh clone to guess."""
        assert self.load("certificate.json")["artefacts_not_committed"] == [
            "longest.trace.tsv"
        ]

    def test_the_artefact_hashes_are_the_files_on_disk(self):
        import hashlib
        from pathlib import Path

        root = Path(self.CERTIFICATE)
        checked = 0
        for name, digest in self.load("certificate.json")["artefacts"].items():
            if not (root / name).exists():
                continue
            actual = hashlib.sha256((root / name).read_bytes()).hexdigest()
            assert actual == digest, name
            checked += 1
        assert checked >= 18, "almost nothing was checked; the paths are wrong"

    def test_nothing_is_absent_that_the_certificate_did_not_declare(self):
        """Anything else missing is a broken certificate, not a space saving."""
        from pathlib import Path

        certificate = self.load("certificate.json")
        root = Path(self.CERTIFICATE)
        missing = {
            name for name in certificate["artefacts"] if not (root / name).exists()
        }
        assert missing <= set(certificate["artefacts_not_committed"]), missing

    def test_the_manifest_holds_nothing_that_varies_between_runs(self):
        """It is the part that has to come out identical on a re-run, from any
        commit, on any machine — so no timestamp and no provenance."""
        manifest = self.load("manifest.json")
        assert "produced" not in json.dumps(manifest)
        ...
        assert set(manifest) == {
            ...
            "counting_proof",
            "decisions",
            "ruled_out_by_lemma_alone",
            "game",
            "artefacts",
            "artefacts_not_committed",
        }

    def test_the_certificate_pins_the_manifest(self):
        import hashlib
        from pathlib import Path

        path = Path(self.CERTIFICATE) / "manifest.json"
        if not path.exists():
            pytest.skip("run scripts/certify.py")
        digest = hashlib.sha256(path.read_bytes()).hexdigest()
        assert self.load("certificate.json")["manifest_sha256"] == digest


class TestCertificateRecordsTheGeneralBound:
    """The bound used to be recorded as `captures=29, quiet_mate=1`.

    That is one profile of one ending. The bound itself does not know how the
    game ends — `P - O <= 88` and `C + T <= 30` — and a certificate that only
    showed the checkmate route left a reader to take the draw case on trust.
    """
    ...
    def test_the_two_global_bounds_are_recorded(self):
        terms = self.certificate()["bound_terms"]
        assert terms["pawn_minus_overlap_bound"] == 88
        assert terms["captures_plus_closing_bound"] == 30
        assert terms["total"] == 118

    def test_both_capture_profiles_are_recorded(self):
        profiles = self.certificate()["bound_terms"]["captures_plus_closing"][
            "profiles"
        ]
        assert profiles["checkmate"]["captures"] == 29
        assert profiles["checkmate"]["closing"] == 1
        assert profiles["all-captured"]["captures"] == 30
        assert profiles["all-captured"]["closing"] == 0
        assert {p["total"] for p in profiles.values()} == {30}

    def test_the_origin_pair_caps_are_recorded_not_the_old_four(self):
        half = self.certificate()["bound_terms"]["pawn_minus_overlap"]
        assert half["unresolved_origin_pair_move_cap"] == 10
        assert half["resolved_origin_pair_move_cap"] == 12
        assert half["pawn_moves_ceiling"] == "min(96, 80 + 2f)"
        assert half["maximiser"] == {
            "resolved_files": 8,
            "pawn_moves": 96,
            "overlaps": 8,
            "value": 88,
    ...
    def test_the_equality_conditions_are_recorded(self):
        equality = self.certificate()["bound_terms"]["equality_conditions"]
        ...
        assert equality["every_pawn_promotes"] is True

    def test_the_case_split_is_recorded_not_just_the_binding_case(self):
        """`S ≥ 3` holds at K = 118 and is false in general, so a certificate
        that recorded only `minimum_switches: 3` would overstate what was
        proved."""
        proof = self.certificate()["counting_proof"]
        assert "false in general" in proof["note"]
        bound = proof["ply_bound"]
        ...
        assert proof["minimum_switches_at_max_k"] == 3

    def test_the_counting_proof_is_recorded_with_every_refutation(self):
        proof = self.certificate()["counting_proof"]
        assert proof["minimum_switches"] == 3
        assert proof["max_plies"] == 17_697
        assert {tuple(r["colours"]) for r in proof["refuted"]} == {
            ("B",),
            ("W",),
            ("B", "W"),
            ("W", "B"),
        ...
        assert all(r["refuted"] for r in proof["refuted"])

    def test_each_refutation_records_how_far_short_the_shape_falls(self):
        """The verdict is derived from these two, so recording only the verdict
        would leave a reader unable to check it."""
        proof = self.certificate()["counting_proof"]
        shortfalls = {" ".join(r["colours"]): r["shortfall"] for r in proof["refuted"]}
        assert shortfalls == {
            "B": 48,
            "W": 48,
            "B W": 42,
        ...
        for entry in proof["refuted"]:
            assert entry["pawn_moves_required"] == 96
            assert entry["pawn_moves_available"] < 96

    def test_only_black_white_black_needs_the_lemma(self):
        """The load-bearing step, and the certificate should say which shape
        it is load-bearing for."""
        proof = self.certificate()["counting_proof"]
        by_lemma = [
            tuple(r["colours"])
            for r in proof["refuted"]
            if r["ground"] == "home-rank lemma"
        ]
        assert by_lemma == [("B", "W", "B")]

    def test_the_origin_pair_search_is_recorded(self):
        lemma = self.certificate()["lemmas"]["origin_pair_cap"]
        assert lemma["max_moves"] == 10
        assert lemma["max_moves_while_both_remain"] == 4

    def test_the_terminal_endpoint_check_is_recorded(self):
        """The seam between `S` over all K endpoints and the critical-move
        actor sequence the refutations are about."""
        lemma = self.certificate()["lemmas"]["terminal_endpoint_free"]
        assert lemma["worst_increase"] <= 0
        assert lemma["shapes_checked"] >= 40

    def test_the_correction_is_recorded_rather_than_quietly_fixed(self):
        caveats = " ".join(self.certificate()["caveats"])
        assert "capped an unresolved origin pair at 4" in caveats
        assert "The correct cap is 10" in caveats

    def test_the_cross_check_is_not_oversold(self):
        """It shares the Shape and the numbers. Calling it a second proof was
        the claim that needed lowering."""
        caveats = " ".join(self.certificate()["caveats"])
        assert "cross-check, not a second proof" in caveats

    def test_every_decision_records_whether_counting_already_killed_it(self):
        decisions = self.certificate()["decisions"]
        counted = {d["shape"] for d in decisions if d["refuted_by_counting"]}
        assert counted == {"B", "W", "B W", "W B", "B W B"}
        assert not any(
            d["cp_sat_feasible"] for d in decisions if d["refuted_by_counting"]
        )
